...
ssize_t pseudo_read(struct file *file, char __user *ubuf, size_t size,
                    loff_t *off) {
  unsigned int rcount, ret, tempcount, pointer=0;
  ...
  while(1)
  {
    if (rcount > buflen)
      {
        pointer = pointer+ buflen;

       ret = copy_to_user(ubuf, (pbuffer+rd_offset), pointer);
       if (ret) {
           printk("copy to user failed\n");
           return -EFAULT;
           }
         printk("readed %d", pointer);
         rd_offset = 0;
         rcount = rcount-pointer;
...
static int __init psuedo_init(void) {
  ...
  // printk("cdev_add ret2 %d",ret2);  giving result 0 on successfull operation
